document.addEventListener("DOMContentLoaded", function () {
  ...
  const datoGuardado = localStorage.getItem("informacionDeUsuario");
  if (datoGuardado) {
      const informacionDeUsuario = JSON.parse(datoGuardado);
      nombre = informacionDeUsuario.nombre;
      apellido = informacionDeUsuario.apellido;
  ...
  document.getElementById("ingreso").addEventListener("click", function () {
      ...
      if (nombre.trim() !== "" && apellido.trim() !== "") {
          localStorage.setItem("informacionDeUsuario", JSON.stringify({ nombre, apellido }));
          document.getElementById("nombreMostrado").textContent = nombre;
          document.getElementById("apellidoMostrado").textContent = apellido;
          ocultarTodasLasPaginas();
          pagina2.style.display = "block";
